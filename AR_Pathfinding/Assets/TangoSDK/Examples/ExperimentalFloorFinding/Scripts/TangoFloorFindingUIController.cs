...
public class TangoFloorFindingUIController : MonoBehaviour 
{
    ...
    public void Update()
    {
        ...
        if (m_pointCloudFloor.m_floorFound && m_pointCloud.m_floorFound)
        {
            ...
            //Set level height
            m_Level.transform.position = new Vector3(m_Level.transform.position.x,  m_pointCloudFloor.transform.position.y, m_Level.transform.position.z);
    ...
    public void OnGUI()
    {
        ...
        if (!m_findingFloor)
        {
            if (GUI.Button(new Rect(Screen.width - 220, 20, 200, 80), "<size=30>Find Floor</size>"))
            {
                ...
                m_Level.transform.position = new Vector3(m_Level.transform.position.x, m_pointCloud.m_floorPlaneY,
                    m_Level.transform.position.z);
                m_Pathfinding.m_YPos = m_pointCloud.m_floorPlaneY;
            }
        }
        else
        {
